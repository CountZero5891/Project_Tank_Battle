...
void exec_v3()
{
	...
	list<Enemy*>  entities;//создаю список, сюда буду кидать объекты.например врагов.
	list<Enemy*>::iterator it;//итератор чтобы проходить по эл-там списка

	list<Bullet*> bullets;
	list<Bullet*>::iterator b_it;
	vector<Object> e = lvl.GetObjects("Enemy");//все объекты врага на tmx карте хранятся в этом векторе
	...
	for (int i = 0; i < e.size(); i++)//проходимся по элементам этого вектора(а именно по врагам)
	{
		//enemies.push_back(new Enemy(EnemyImage, "Enemy", lvl, e[i].rect.left, e[i].rect.top, 71, 80));
		entities.push_back(new Enemy(EnemyImage, "Enemy", lvl, e[i].rect.left, e[i].rect.top, 71, 80, 1));
		//entities.push_back(new Bullet(BulletImage, "Bullet", lvl, e[i].rect.left, e[i].rect.top, 16, 16, p.state));
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (p.isShoot == true)
			{
				p.isShoot = false;
				cout << p.state << endl;
				bullets.push_back(new Bullet(BulletImage, "Bullet", lvl, p.x, p.y, 16, 16, p.state));
			}//если выстрелили, то появляется пуля.
			if (event.type == Event::KeyPressed)
			{
				if ((event.key.code == Keyboard::Escape))
		...
		for (b_it = bullets.begin(); b_it != bullets.end();)//говорим что проходимся от начала до конца
		{
			(*b_it)->update(time);//вызываем ф-цию update для всех объектов (по сути для тех, кто жив)
			if ((*b_it)->life == false) // если этот объект мертв, то удаляем его
			{
...
//					cout << (*it)->dirr << endl;
					bullets.push_back(new Bullet(BulletImage, "EnemyBullet", lvl, (*it)->x, (*it)->y, 16, 16, (*it)->direction));
				}
			...
			///тут идет уже проверка на контакт врага с пулей, если пуля коснулась врага здоровье его уменьщшается на 50 единиц
			for (list<Enemy*>::iterator it2 = entities.begin(); it2 != entities.end(); it2++)
			{
					Entity* enemy = *it;
					for (list<Bullet*>::iterator it2 = bullets.begin(); it2 != bullets.end(); it2++)
					{
						Bullet* bullet = *it2;
